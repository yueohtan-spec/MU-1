...
function handleDroppedItem(droppedItem) {
  ...
  if (!currentItem || newCP > oldCP) {
    ...
    addLog(`[NÂNG CẤP] Trang bị mới [${droppedItem.name}] (CP: ${newCP}) mạnh hơn -> Tự động mặc vào!${sellMsg}`, 'log-equip');
    ...
    state.zen += sellZen;
    addLog(`[RƠI ĐỒ] Nhặt [${droppedItem.name}] (CP: ${newCP} thấp hơn) -> Tự động bán thu về +${sellZen.toLocaleString()} Zen (30%)!`, 'log-zen');
  }
}
...
function updateUI() {
  const stats = calculateStats();
  document.getElementById('heroName').innerText = state.gender === 'male' ? `${state.username} [DK Nam]` : `${state.username} [FE Nữ]`;
...
function updateRebirthTab() {
  ...
  document.getElementById('chkLevel').innerHTML = `Cấp độ yêu cầu: <b>Lv.${req.reqLevel}</b> (Hiện tại: Lv.${state.level}) <span class="${passLevel ? 'check-pass' : 'check-fail'}">${passLevel ? '✔ ĐẠT' : '✖ CHƯA ĐỦ'}</span>`;
  document.getElementById('chkZen').innerHTML = `Phí Zen Hardcore: <b>${req.reqZen.toLocaleString()} Zen</b> (Có: ${state.zen.toLocaleString()}) <span class="${passZen ? 'check-pass' : 'check-fail'}">${passZen ? '✔ ĐẠT' : '✖ THIẾU'}</span>`;
  
  let jewelTxt = `${req.reqBless} Bless`;
  if (req.reqChaos > 0) jewelTxt += `, ${req.reqChaos} Chaos`;
  if (req.reqLife > 0) jewelTxt += `, ${req.reqLife} Life`;

  document.getElementById('chkJowels').innerHTML = `Ngọc tế lễ: <b>${jewelTxt}</b> <span class="${passJewels ? 'check-pass' : 'check-fail'}">${passJewels ? '✔ ĐẠT' : '✖ THIẾU'}</span>`;

  document.getElementById('rsInfoText').innerHTML = `
...
function renderDetailPanel() {
  ...
  if (!item) {
    dTitle.innerText = "Ô trang bị đang trống";
    dTitle.style.color = "#8fa0b8";
    dStats.innerText = "Khi đánh quái nhặt được món đồ phù hợp, hệ thống sẽ tự động trang bị vào đây.";
    dOptions.innerHTML = "";
  ...
  dLockTag.innerText = `Lực chiến: ${cp.toLocaleString()}`;
